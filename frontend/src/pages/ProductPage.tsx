...
export function ProductPage() {
    ...
    async function handleAdd() {
        ...
        if (token) {
            ...
            toast("Добавлено в корзину (гость). Оформите заказ в корзине.");
            setAdded(true);
            setTimeout(() => setAdded(false), 2000);
        }
    }
    ...
    async function submitQuestion(e: React.FormEvent) {
        ...
        try {
            ...
            setQText("");
            toast("Вопрос отправлен продавцу.");
    ...
    async function submitReview(e: React.FormEvent) {
        ...
        try {
            ...
            setRevText("");
            setRevRating(5);
            toast("Спасибо, отзыв опубликован.");
            void refetchReviews();
        }
        catch (e) {
            toast(getErrorMessage(e as never), false);
    ...
    return (<div className="product-page">
      ...
      <section className="product-hero">
        <div className="product-hero__media">
          {data.imageUrl ? (
            <button type="button" className="product-hero__image-btn" onClick={() => setLightbox(true)} aria-label="Открыть фото">
        ...
        <div className="product-hero__content">
          ...
          <div className="product-hero__actions">
            <label className="field" style={{ margin: 0, width: "100px" }}>
              <span>Кол-во</span>
              <input type="number" className="input--qty" min={1} max={data.stock} value={qty} onChange={(e) => setQty(Math.max(1, Math.min(data.stock, Number(e.target.value))))}/>
            </label>
            <button type="button" className={added ? "btn btn--ghost" : "btn btn--primary"} disabled={adding || data.stock < 1} onClick={handleAdd}>
      ...
      {lightbox && data.imageUrl && (
        <div className="lightbox" onClick={() => setLightbox(false)} role="dialog" aria-modal="true" aria-label="Фото товара">
          <img className="lightbox__img" src={data.imageUrl} alt={data.name} onClick={(e) => e.stopPropagation()}/>
        </div>
      )}
    </div>);
}
